...
const addBlog = asyncHandler(async (req, res) => {
  try {
    ...
    const blog = await Blog.create({
      ...
      isPublished: userType === "admin" ? isPublished : false, // Users' blogs need approval
      authorType: userType,
      authorEmail: email,
      authorName: name,
    });
...
const togglePublish = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, { blog }, "Blog Publish status toggled Successfully")
    );
});
...
const getAllBlogsAdmin = asyncHandler(async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json(
        new ApiResponse(200, blogs, "All Blogs Fetched Successfully (Admin)")
      );
  } catch (error) {
    throw new ApiError(
      500,
